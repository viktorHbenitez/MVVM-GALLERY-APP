...
extension PhotoListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: PhotoListTableViewCell.identifierCell , for: indexPath) as? PhotoListTableViewCell else {
...
class PhotoListTableViewCell: UITableViewCell {
    ...
    var photoListCellViewModel : CellInterface? {
        didSet {
            ...
            // Use the library to store image in the cache
            mainImageView?.sd_setImage(with: URL( string: photoListCellViewModel?.imgURL ?? "" ), completed: nil)
            dateLabel.text = photoListCellViewModel?.date
        }
    }
}
